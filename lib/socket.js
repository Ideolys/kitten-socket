  ...
  function startServer (callback) {
    ...
    _server.listen(_port, _host, function () {
      ...
      if (fs.existsSync(_logsFilename)) {
        ...
        catch (e) {} // do not care of empty file or non JSON value
      }
      else {
    ...
    _server.on(_eventName, function (socket) {
      ...
      socket.setKeepAlive(true, 300000); // check for tcp probe each 5min
      ...
      socket.on('error', function (err) {
        console.log('Server: Client socket ' + socket.id + ' error: ', err);
      });
      // Set timeout to 2s
    ...
    for (var i = 0; i < _clients.length; i++) {
      if (_clients[i].uid === packet.data.uid) {
        ...
        if (_token && _token === packet.data.token) {
          _clients[i].socket.emit('error', `New client token mismatched with registered one (${packet.data.uid})`);
        }
